...
export function DashboardFilters({ value, options, onChange }: Props) {
  ...
  const dirty = Boolean(value.period || value.modality || value.courseId || value.status || value.query);
  return <section className="filter-panel" aria-label="Filtros">
    ...
    <label><span>Situação</span><select value={value.status} onChange={(event) => patch("status", event.target.value)}><option value="">Todas</option><option value="CRITICAL,NEVER">Acesso crítico</option><option value="PENDING">Estrutura pendente</option><option value="DELIVERED_LATE">Entregue com atraso</option><option value="NOT_VERIFIABLE">Não verificável</option><option value="NOT_APPLICABLE">Não aplicável</option><option value="INHERITED_READY">Estrutura herdada</option><option value="CURRENT">Acesso em dia</option></select></label>
