...
export function ExecutiveOverview({ data, onOpen }: { data: DashboardData; onOpen: (row: AssignmentRow) => void }) {
  ...
  return <div className="executive-grid">
    <section className="insight-section trend-section"><div className="section-heading compact"><div><span className="eyebrow"><TrendingDown size={14} />Tendência semanal</span><h2>Evolução das exceções</h2></div></div><Suspense fallback={<div className="chart-loading">Carregando evolução…</div>}><TrendChart data={data.trend} /></Suspense></section>
    <section className="insight-section"><div className="section-heading compact"><div><span className="eyebrow"><BarChart3 size={14} />Comparativo</span><h2>Situação por modalidade</h2></div></div><div className="modality-list">{data.modalityBreakdown.map((item) => { const percent = item.total ? Math.round(item.compliant / item.total * 100) : 0; return <div key={item.modality}><div><strong>{item.label}</strong><span>{item.issues} ponto{item.issues === 1 ? "" : "s"} crítico{item.issues === 1 ? "" : "s"}</span></div><div className="progress-track"><span style={{ width: `${percent}%` }} /></div><b>{percent}% regular</b></div>; })}</div></section>
