...
function structureStatus(row: AssignmentRow) {
  return row.requirements.find((item) => item.status === "NOT_VERIFIABLE")?.status
    || row.requirements.find((item) => item.status === "PENDING" && item.overdue)?.status
