...
function structureStatus(row: AssignmentRow) {
  const requirements = row.requirements.filter((item) => item.responsibility !== 'OTHER_TEACHER');
  if (!requirements.length) return 'NOT_APPLICABLE';
  return requirements.find((item) => item.status === "PENDING" && item.overdue)?.status
    || (requirements.some((item) => item.status === "PENDING") ? "WITHIN_DEADLINE" : null)
    || requirements.find((item) => item.status === "NOT_VERIFIABLE")?.status
    || requirements.find((item) => item.status === "DELIVERED_LATE")?.status
    || requirements.find((item) => item.status === "INHERITED_READY")?.status
    || requirements.find((item) => item.status === "NOT_APPLICABLE")?.status
    || "DELIVERED_ON_TIME";
}
...
export function ExceptionTable({ data, filters, onFilters, onOpen }: Props) {
  return <section className="data-section">
    <div className="section-heading"><div><span className="eyebrow"><ListFilter size={14} />Acompanhamento</span><h2>Docentes e disciplinas</h2></div><strong className="result-count">{data.pagination.total} resultado{data.pagination.total === 1 ? "" : "s"}</strong></div>
